...
public class TopologyInstance {
	...
	protected Map<DatapathId, Set<OFPort>> switchPorts; // Set of ports for each
														// switch
	/**
	...
	protected Map<DatapathId, Cluster> switchClusterMap; // switch to OF domain
															// map
	...
	// Set of all links organized by node port tuple. Note that switchPortLinks
	// does not contain all links of multi-cluster topology.
	protected Map<NodePortTuple, Set<Link>> allLinks;
	// Set of all ports organized by DatapathId. Note that switchPorts map
	// contains only ports with links.
	...
	protected class PathCacheLoader extends CacheLoader<RouteId, Route> {
		...
		public Route load(RouteId rid) {
			return ti.buildL2Path(rid);
		}
	}

	protected class MultipathCacheLoader extends
		...
		public ArrayList<Route> load(RouteId rid) {
			return ti.buildL2Paths(rid);
		}
	}

	private final MultipathCacheLoader multipathCacheLoader = new MultipathCacheLoader(
			this);
	...
	public void compute() {
		...
		// Step 5. Compute broadcast tree for the whole topology (needed to
		// avoid loops).
		// Cost for tunnel links are high to discourage use of
		// tunnel links. The cost is set to the number of nodes
		// in the cluster + 1, to use as minimum number of
		...
		// Step 6. Compute set of ports for broadcasting. Edge ports are
		// included.
	...
	public void printTopology() {
		...
		log.debug("Destination Rooted Full Trees: {}",
				destinationRootedFullTrees);
	...
	private long dfsTraverse(long parentIndex, long currIndex,
			DatapathId currSw, Map<DatapathId, ClusterDFS> dfsList,
			Set<DatapathId> currSet) {
	...
	/*
	 * Dijkstra that calculates destination rooted trees over the entire
	 * topology.
	 */
	protected BroadcastTree dijkstra(Map<DatapathId, Set<Link>> links,
			DatapathId root, Map<Link, Integer> linkCost, boolean isDstRooted) {
		HashMap<DatapathId, Link> nexthoplinks = new HashMap<DatapathId, Link>();
		HashMap<DatapathId, Integer> cost = new HashMap<DatapathId, Integer>();
		int w;
	...
	/*
	 * Modification of the calculateShortestPathTreeInClusters (dealing with
	 * whole topology, not individual clusters)
	 */
	public void calculateAllShortestPaths() {
		...
		if (this.destinationRootedFullTrees.size() > 0) {
			this.finiteBroadcastTree = destinationRootedFullTrees.values()
					.iterator().next();
		}
	}
			...
			for (OFPort p : this.allPorts.get(sw)) {
				...
				if (isEdge(sw, p) || broadcastNodePorts.contains(npt)) {
					if (broadcastPortMap.containsKey(sw)) {
						...
						broadcastPortMap.put(sw,
								new HashSet<OFPort>(Arrays.asList(p)));
					}
				}
			}
		}
	...
	protected Route buildL2Path(RouteId id) {
		NodePortTuple npt;
		...
		Map<DatapathId, Link> nexthoplinks = destinationRootedFullTrees.get(
				dstId).getLinks();
	...
	protected ArrayList<Route> buildL2Paths(RouteId rid) {
	...
	protected boolean l2PathExists(DatapathId srcId, DatapathId dstId) {
		BroadcastTree bt = destinationRootedTrees.get(dstId);
		if (bt == null)
			return false;
	 ...
	 */
	protected Route getL2Path(ServiceChain sc, DatapathId srcId, OFPort srcPort,
		...
		NodePortTuple npt;
		Route r = getL2Path(srcId, dstId, U64.of(0));
	...
	protected Route getL2Path(DatapathId srcId, DatapathId dstId, U64 cookie) {
	...
	protected ArrayList<Route> getL2Paths(DatapathId srcId, DatapathId dstId, PathOrderingCriteria criteria) {
	...
	protected Route getL2Multipath(DatapathId srcId, OFPort srcPort, DatapathId dstId, OFPort dstPort, PathOrderingCriteria criteria, Integer numberOfMultipaths) {
		
		if (srcId.equals(dstId))
			return null;
		
		List<NodePortTuple> nptList;
		List<Route> paths = getL2Paths(srcId, dstId, criteria);
	...
	protected boolean isInternalToOpenflowDomain(DatapathId switchid,
			OFPort port) {
	...
	protected boolean inSameOpenflowDomain(DatapathId switch1,
			DatapathId switch2) {
	 ...
	 */
	protected boolean isIncomingBroadcastAllowedOnSwitchPort(DatapathId sw,
			OFPort portId) {
		if (!isEdge(sw, portId)) {
	...
	public boolean inSameBroadcastDomain(DatapathId s1, OFPort p1,
